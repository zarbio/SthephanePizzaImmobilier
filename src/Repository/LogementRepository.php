...
class LogementRepository extends ServiceEntityRepository
{
    ...
    public function add(Logement $entity, bool $flush = false): void
    {
    ...
    public function sortByCriterias( $superficie, $nombrePieces, $typeLogement, $adresse, $piscine, $exterieur, $garage, $typeVente, $prix,$prixMax)
    {
        $qb = $this->createQueryBuilder('l')
            ->where('l.adresse like :adresse')
            ->setParameter('adresse','%'.$adresse.'%')
            ->andWhere('l.piscine = :piscine')
            ->setParameter('piscine',$piscine);
            if ($prix != null)
            {

                $qb->andWhere('l.prix > :prixMin')
                ->setParameter('prixMin',$prix);
            }
            if ($prixMax != null)
            {
